...
def get_access_token(consumer_key, consumer_secret, force_refresh=False):
    ...
    if not force_refresh and _token_cache["token"] and _token_cache["expires_at"]:
        if datetime.now() < _token_cache["expires_at"]:
            print("[M-PESA] Using cached access token")
            return _token_cache["token"]
    
    api_url = "https://sandbox.safaricom.co.ke/oauth/v1/generate?grant_type=client_credentials"
    
    try:
        print("[M-PESA] Generating new access token...")
        ...
        print("[M-PESA] Token generated and cached successfully")
        
        return token
    except requests.Timeout:
        print("[M-PESA] Timeout while generating access token")
        return None
    except requests.RequestException as e:
        print(f"[M-PESA] Error generating access token: {e}")
        return None
...
def initiate_stk_push(phone_number, amount, account_reference="TindiTech", transaction_desc="Order Payment"):
    ...
    for attempt in range(max_retries):
        try:
            print(f"[M-PESA] STK Push attempt {attempt + 1}/{max_retries}")
            ...
            if response.status_code == 200 and response_data.get('ResponseCode') == '0':
                print("[M-PESA] STK Push successful")
                return {
                    "success": True, 
